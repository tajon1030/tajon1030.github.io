...
세션/쿠키방식은 서버가 세션을 저장하고 관리하는 방식이기때문에 서버 증설시에 문제가 발생할 수 있다. (이럴경우 redis를 이용하여 세션 클러스터링을 적용하면 되긴 하다.) 이런 세션/쿠키 인증방식은 시간적 비효율성이 발생한다는 단점이 있기때문에 헤더에 토큰값만 전달하면 되고 세션 서버에 조회하는 시간이 발생하지 않는 jwt인증방식을 사용한다.
